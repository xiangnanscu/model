...
local User = Model:create_model {
  table_name = 'user', -- test pg reserved word
  fields = {
...
local Author = Model:create_model {
  table_name = 'author',
  fields = {
    { "name", "姓名", maxlength = 200, unique = true },
    { "email", type = 'email' },
...
mdesc("User 模型测试", function()
  mit("表名是escape了的", function()
    local user = User:create { username = 'testuser', password = 'password' }
    assert.are.same(user.username, 'testuser')
    assert.are.same(user.password, 'password')
  end)
end)
