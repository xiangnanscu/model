...
function escape_like_value(val) {
  val = String(val);
  val = val.replace(/([\\%_])/g, "\\$1").replace(/'/g, "''");
  return val;
}
const EXPR_OPERATORS = {
  ...
  contains: (key, value) => {
    const esc = escape_like_value(value);
    return `${key} LIKE '%${esc}%' ESCAPE '\\'`;
  },
  icontains: (key, value) => {
    const esc = escape_like_value(value);
    return `${key} ILIKE '%${esc}%' ESCAPE '\\'`;
  },
...
const extract_column_name = (sql_part) => {
  // 1. T.col, user.name format
  const table_column_match = sql_part.match(/^(\w+)\.(\w+)$/);
  if (table_column_match) {
    return table_column_match[2]; // Return column name part
  }

  // 2. T.col AS alias, col AS alias format
  const alias_match = sql_part.match(/[Aa][Ss]\s+(\w+)\s*$/);
  if (alias_match) {
    return alias_match[1]; // Return alias
  }

  // 3. Ignore function calls (cases containing parentheses)
  if (sql_part.match(/\([^)]*\)/)) {
    return null;
  }

  // 4. Direct column name
...
class Sql {
  ...
  _get_update_token(row, columns) {
    ...
    const auto_now = this.model.auto_now_name;
    if (auto_now) {
      kv.push(`${auto_now} = CURRENT_TIMESTAMP`);
    }
    return kv.join(", ");
  }
  ...
  _get_bulk_upsert_token(rows, key, columns) {
    [rows, columns] = this._get_bulk_insert_values_token(rows, columns);
    const insert_token = `(${as_token(columns)}) VALUES ${as_token(rows)} ON CONFLICT (${as_token(
      key,
    )})`;
    if ((typeof key === "object" && key.length === columns.length) || columns.length === 1) {
      return `${insert_token} DO NOTHING`;
  ...
  _set_select_subquery_upsert_token(rows, key, columns) {
    const insert_token = `(${as_token(columns)}) ${rows.statement()} ON CONFLICT (${as_token(
      key,
    )})`;
    if ((typeof key === "object" && key.length === columns.length) || columns.length === 1) {
  ...
  _get_order_column(key) {
    if (typeof key !== "string") {
      ...
      if (matchResult) {
        ...
        if (this._nulls_first) {
          nulls_clause = " NULLS FIRST";
        } else if (this._nulls_last) {
          nulls_clause = " NULLS LAST";
        }

        return `${parsedColumn} ${order}${nulls_clause}`;
      } else {
  ...
  _parse_column(key, context) {
        ...
        if (reversed_field) {
          ...
          if (!prefix) {
            const join_cond_cb = (ctx) => {
              ...
              return `${ctx[left_model_index][reversed_field.reference_column]} = ${
    ...
    if (json_keys) {
      if (json_keys.length > 0) {
        final_column = `${prefix + "." + smart_quote(column)} #> [${as_literal_without_brackets(
          json_keys,
        )}]`;
      }
      if (op === "contains") {
  ...
  _resolve_field_builder(f) {
    if (typeof f !== "object") {
      ...
      return `(${this._resolve_field_builder(f.left)} ${f.operator} ${this._resolve_field_builder(
        f.right,
      )})`;
    }
  }
   ...
   * @param {string|Array} cols - Column name or array of column names
   * @param {Array|Sql} range - Array of values or subquery
   * @returns {Sql} Returns current instance for method chaining
   */
  where_in(cols, range) {
  ...
  /**
   * Add other statements before SQL statement
   * @param {...(string|Sql)} varargs - SQL statements or Sql objects to add
   * @returns {Sql} Returns current instance for method chaining
   */
  prepend(...varargs) {
    if (!this._prepend) {
  ...
  /**
   * Add other statements after SQL statement
   * @param {...(string|Sql)} varargs - SQL statements or Sql objects to add
   * @returns {Sql} Returns current instance for method chaining
   */
  append(...varargs) {
  ...
  /**
   * Set NULL values to be sorted first
   * @returns {Sql} Returns current instance for method chaining
   */
  nulls_first() {
    this._nulls_first = true;
    this._nulls_last = false; // Clear the opposite setting
    return this;
  }

  /**
   * Set NULL values to be sorted last
   * @returns {Sql} Returns current instance for method chaining
   */
  nulls_last() {
    this._nulls_last = true;
    this._nulls_first = false; // Clear the opposite setting
    return this;
  }
  ...
  where(cond, op, dval) {
    if (typeof cond === "object") {
      if (!cond.__IS_LOGICAL_BUILDER__) {
        const where_token = this._get_condition_token_from_table(cond);
        return this._handle_where_token(where_token, "(%s) AND (%s)");
      } else {
  ...
  /**
   * Increase value of specified column
   * @param {string|Object} name - Column name or object containing column names and increment values
   * @param {number} [amount=1] - Value to increase by
   * @returns {Sql} Returns current instance for method chaining
   */
  ...
  /**
   * Decrease value of specified column
   * @param {string|Object} name - Column name or object containing column names and decrement values
   * @param {number} [amount=1] - Value to decrease by
   * @returns {Sql} Returns current instance for method chaining
   */
  ...
  /**
   * Add annotations
   * @param {Object} kwargs - Key-value pairs for annotations
   * @returns {Sql} Returns current instance for method chaining
   */
  annotate(kwargs) {
    if (!this._annotate) {
  ...
  /**
   * Insert data
   * @param {Object|Sql} rows - Data to insert or subquery
  ...
  /**
   * Align data
   * @param {Object[]} rows - Data to align
   * @param {string|string[]} key - Primary key column name
   * @param {string[]} [columns] - Columns to align
   * @returns {Sql} Returns current instance for method chaining
   */
  ...
  /**
   * Update data
   * @param {Object|Sql} row - Data to update or subquery
  ...
  /**
   * Merge data
   * @param {Object[]} rows - Data to merge
   * @param {string|string[]} key - Primary key column name
   * @param {string[]} [columns] - Columns to merge
   * @returns {Sql} Returns current instance for method chaining
   */
   ...
   * @param {Object[]|Sql} rows - Data to insert or update, or subquery
   * @param {string|string[]} [key] - Primary key column name
   * @param {string[]} [columns] - Columns to operate on
   * @returns {Sql} Returns current instance for method chaining
   */
   ...
   * @param {Object[]|Sql} rows - Data to update or subquery
   * @param {string|string[]} [key] - Primary key column name
   * @param {string[]} [columns] - Columns to update
   * @returns {Sql} Returns current instance for method chaining
   */
   ...
   * @param {any[]} keys - Array of primary key values
   ...
   * @returns {Promise<any>} Returns execution result
   */
  async exec_statement(statement) {
    let records = await this.model.query(statement, { compact: this._compact });
    let all_results;
    if (this._prepend) {
      all_results = records;
   ...
   * @param {string} [op] - Operator
   * @param {*} [dval] - Default value
   * @returns {Promise<number>} Returns record count
   */
   ...
   * @returns {Promise<boolean>} Returns whether records exist
   */
  async exists() {
    const statement = `SELECT EXISTS (${this.select(1).limit(1).compact().statement()})`;
    const [res, err] = await this.model.query(statement, { compact: this._compact });
    if (res === undefined) {
      throw new Error(err);
    } else {
      return res[0][0];
  ...
  /**
   * Set whether to return raw data
   * @param {boolean} [is_raw=true] - Whether to return raw data
   * @returns {Sql} Returns current instance for method chaining
   */
  raw(is_raw) {
  ...
  /**
   * Set whether to commit transaction
   * @param {boolean} [bool=true] - Whether to commit transaction
   * @returns {Sql} Returns current instance for method chaining
   */
  commit(bool) {
    if (bool === undefined) {
   ...
   * @param {boolean} [bool=true] - Whether to skip validation
   * @returns {Sql} Returns current instance for method chaining
   */
  skip_validate(bool) {
    if (bool === undefined) {
  ...
  /**
   * Flatten results
   * @param {string} [col] - Column to flatten
   * @returns {Promise<any>} Returns flattened result
   */
  async flat(col) {
    if (col) {
   ...
   * @param {string} [op] - Operator
   * @param {*} [dval] - Default value
   * @returns {Promise<any|false>} Returns single record or false
   */
   ...
   * @param {string} [op] - Operator
   * @param {*} [dval] - Default value
   * @returns {Promise<any>} Returns single record
   * @throws {Error} If record does not exist or multiple records returned
   */
  ...
  /**
   * Set whether to execute in compact mode
   * @returns {Sql} Returns current instance for method chaining
   */
  compact() {
    this._compact = true;
  ...
  /**
   * Select related labels
   * @param {string[]} [names] - Field names to select
   * @returns {Sql} Returns current instance for method chaining
   */
   ...
   * @param {string|Object} fk_name - Foreign key field name or foreign key field object
   * @param {string|string[]} [select_names] - Field names to select
   * @param {string} [more_name] - More field names to select
   * @param {...string} varargs - More field names to select
   * @returns {Sql} Returns current instance for method chaining
   */
  ...
  /**
   * Recursive select
   * @param {string} name - Foreign key field name
   * @param {*} value - Value
   * @param {string[]} [select_names] - Field names to select
   * @returns {Sql} Returns current instance for method chaining
   */
...
export default new Proxy(Sql, {
  apply(target, thisArg, argumentsList) {
    return target.new(...argumentsList);
  },
});
export { Sql };
