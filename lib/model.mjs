...
// (\w+)\.__SQL_BUILDER__ => $1 instanceof Sql
...
Sql.prototype.merge_gets = function (rows, key) {
  let columns = get_keys(rows);
  [rows, columns] = this._get_cte_values_literal(rows, columns, true);
  const join_cond = this._get_join_condition_from_key(key, "V", this._as || this.table_name);
  const cte_name = `V(${columns.join(", ")})`;
...
Sql.prototype.select_related = function (fk_name, select_names, ...varargs) {
  const fk = this.model.foreignkey_fields[fk_name];
  if (fk === undefined) {
  ...
  if (Array.isArray(select_names)) {
    ...
    throw new Error(`invalid argument type ${typeof select_names} for select_related`);
  }
...
Xodel._make_model_class = function (opts) {
  ...
  ModelClass.foreignkey_fields = {};
  ModelClass.names = [];
  ModelClass.detail_names = [];
  ...
  this.resolve_foreignkey_self.call(proxy);
  return proxy;
};
...
Xodel.resolve_foreignkey_self = function () {
...
Xodel.filter_with_fk_labels = async function (kwargs) {
  const records = this.create_sql().select_related_labels().where(kwargs);
