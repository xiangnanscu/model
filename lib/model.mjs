...
class Model {
  constructor(options) {
    const tasks = this.constructor.get_async_tasks(options);
    if (tasks.length > 0) {
      return Promise.all(tasks).then(() => this.constructor.create_model(options));
    } else {
  ...
  static async get_http_model(model_key) {
    if (!this.http_model_cache[model_key]) {
      ...
      this.http_model_cache[model_key] = await this(data);
    } else {
