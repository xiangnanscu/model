...
class BaseField {
  ...
  static normalize_field_shortcuts = normalize_field_shortcuts;
  ...
  constructor(options) {
    ...
    if (this.autocomplete) {
      if (this.max_choices_count === undefined) {
        this.max_choices_count = Number(process.env.MAX_CHOICES_COUNT) || 100;
      }
      if (this.max_display_count === undefined) {
  ...
  get_options() {
    ...
    for (const name of this.get_option_names()) {
      if (Object.getOwnPropertyDescriptor(this, name)?.value !== undefined) {
        ret[name] = this[name];
      }
    }
    if (ret.attrs) {
...
StringField.prototype.compact = true;
StringField.prototype.trim = true;
...
class TextField extends BaseField {
  static option_names = ["pattern", "length", "minlength", "maxlength", "image_base64"];
  constructor(options) {
...
class IdCardField extends StringField {
  constructor(options) {
    super({ type: "id_card", db_type: "varchar", length: 18, ...options });
  }
  get_validators(validators) {
    validators.unshift(Validator.id_card);
    return super.get_validators(validators);
  }
}
...
class BooleanField extends BaseField {
  ...
  constructor(options) {
    super({ type: "boolean", db_type: "boolean", ...options });
...
DatetimeField.prototype.precision = 0;
DatetimeField.prototype.timezone = true;

class DateField extends BaseField {
  constructor(options) {
    super({ type: "date", db_type: "date", ...options });
  }
  get_validators(validators) {
...
class TimeField extends BaseField {
  ...
  constructor(options) {
    super({ type: "time", db_type: "time", ...options });
  }
  get_validators(validators) {
...
TimeField.prototype.precision = 0;
TimeField.prototype.timezone = true;
...
ForeignkeyField.prototype.FK_TYPE_NOT_DEFIEND = FK_TYPE_NOT_DEFIEND;
ForeignkeyField.prototype.on_delete = "CASCADE";
ForeignkeyField.prototype.on_update = "CASCADE";
ForeignkeyField.prototype.admin_url_name = "admin";
ForeignkeyField.prototype.models_url_name = "model";
ForeignkeyField.prototype.keyword_query_name = "keyword";
ForeignkeyField.prototype.limit_query_name = "limit";
ForeignkeyField.prototype.convert = String;
...
class ArrayField extends BaseArrayField {
  ...
  constructor(options) {
    ...
    if (this.field) {
      this.field = normalize_field_shortcuts(this.field);
      if (!this.field.name) {
...
class TableField extends BaseArrayField {
  ...
  load(rows) {
    if (!Array.isArray(rows)) {
      throw new Error("value of table field must be array, not " + typeof rows);
    }
...
class AliossField extends StringField {
  ...
  constructor(options) {
    super({ type: "alioss", db_type: "varchar", maxlength: 1024, ...options });
    this.setup(options);
  }
  setup(options) {
    ...
    this.upload_url = options.upload_url || ALIOSS_URL;
  }
  get_options() {
  ...
  load(value) {
    if (value && value.startsWith("/")) {
      const scheme = process.env.VITE_HTTPS === "on" ? "https" : "http";
      return scheme + ":" + value;
    } else {
      return value;
    }
