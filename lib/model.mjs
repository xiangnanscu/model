  ...
  if (typeof join_args === "object") {
    ...
    if (fk) {
      return this._base_join(
        ...
        fk.reference,
        (ctx) =>
          `${ctx[this.model.table_name][join_args]} = ${ctx[fk.reference.table_name][fk.reference_column]}`,
      );
    } else {
...
Sql.prototype.get_or_create = async function (params, defaults, columns) {
  ...
  }(${insert_columns_token}) SELECT ${as_literal_without_brackets(values_list)} WHERE NOT EXISTS (${this.model
    .create_sql()
    .select(1)
    .where(params)}) RETURNING ${all_columns_token})`;
  const inserted_set = this.as("new_records")
    .with(`new_records(${all_columns_token})`, insert_sql)
    ._base_select(all_columns_token)
    ._base_select("TRUE AS __is_inserted__");
  const selected_set = this.model
    .create_sql()
    .where(params)
    ._base_select(all_columns_token)
